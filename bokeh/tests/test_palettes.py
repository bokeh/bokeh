...
def ret_ns_key (string):
    ...
    return [string[:-len(num)], int(num)]

def test_for_palettes():
    palettes = [x for x in dir(pal) if isinstance(getattr(pal,x),list)]
    # __palettes__ is also return and should not be tested
    palettes = palettes[:-1]
    assert sorted(palettes, key=ret_ns_key) == sorted(pal.__palettes__, key=ret_ns_key)
...
def test_palette_lengths():
    ...
    for palstr in palettes:
        assert len(getattr(pal,palstr)) == int(re.findall('\d+',palstr)[-1])
...
def test_palette_contents_hard():
    ...
    # Compares palettes in test_palettes.py to Bokeh palettes in palettes.py
    pals = {
    ...
    # __palettes__ is also return and should not be tested
    palettes = palettes[:-1]
    assert len(palettes) == len(pals)
    for palstr in palettes:
        assert getattr(pal,palstr) == pals[palstr]
