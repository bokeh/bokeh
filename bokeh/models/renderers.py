...
class GlyphRenderer(DataRenderer):
    ...
    def _check_cdsview_source(self):
        if self.data_source is not self.view.source:
            return str(self)
    ...
    def _check_bad_column_name(self):
        ...
        if not self.data_source:
            return
        if isinstance(self.data_source, WebSource):
            return
        missing_values = set()
        ...
        for name, item in self.glyph.properties_with_values(
            ...
            if not isinstance(item, dict):
                continue
            if not isinstance(self.data_source, ColumnDataSource):
                continue
