...
class BokehPropDirective(BokehDirective):
    ...
    def run(self):
        ...
        try:
            ...
            raise SphinxError(
                "Could not generate reference docs for %r: could not import module %r"
                % (self.arguments[0], self.options["module"])
            )
        ...
        if model is None:
            raise SphinxError(
                ...
                % (self.arguments[0], model_name, self.options["module"])
            )
        ...
        try:
            ...
            raise SphinxError(
                "Unable to generate reference docs for %s, no property '%s' in %s"
                % (self.arguments[0], prop_name, model_name)
            )
