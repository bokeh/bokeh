...
BAD_COLUMN_NAME                                    = Error(1001, "Glyph refers to nonexistent column name. This could either be due to a misspelling or typo, or due to an expected column being missing. ") # NOQA
MISSING_GLYPH                                      = Error(1002, "Glyph renderer has no glyph set")
NO_SOURCE_FOR_GLYPH                                = Error(1003, "Glyph renderer has no data source")
REQUIRED_RANGE                                     = Error(1004, "A required Range object is missing")
MISSING_GOOGLE_API_KEY                             = Error(1005, "Google now requires API keys for all Google Maps usage")
NON_MATCHING_DATA_SOURCES_ON_LEGEND_ITEM_RENDERERS = Error(1006, "LegendItem.label is a field, but renderer data sources don't match")
MISSING_MERCATOR_DIMENSION                         = Error(1007, "Mercator Tickers and Formatters must have their dimension property set to 'lat' or 'lon'")
REQUIRED_SCALE                                     = Error(1008, "A required Scale object is missing")
INCOMPATIBLE_SCALE_AND_RANGE                       = Error(1009, "A Scale is incompatible with one or more ranges on the same plot dimension")
CDSVIEW_SOURCE_DOESNT_MATCH                        = Error(1010, "CDSView used by Glyph renderer must have a source that matches the Glyph renderer's data source")
MALFORMED_GRAPH_SOURCE                             = Error(1011, "The GraphSource is incorrectly configured")
INCOMPATIBLE_MAP_RANGE_TYPE                        = Error(1012, "Map plots can only support Range1d types, not data ranges")
INCOMPATIBLE_POINT_DRAW_RENDERER                   = Error(1013, "PointDrawTool renderers may only reference XYGlyph models")
INCOMPATIBLE_BOX_EDIT_RENDERER                     = Error(1014, "BoxEditTool renderers may only reference Rect glyph models")
INCOMPATIBLE_POLY_DRAW_RENDERER                    = Error(1015, "PolyDrawTool renderers may only reference MultiLine and Patches glyph models")
INCOMPATIBLE_POLY_EDIT_RENDERER                    = Error(1016, "PolyEditTool renderers may only reference MultiLine and Patches glyph models")
INCOMPATIBLE_POLY_EDIT_VERTEX_RENDERER             = Error(1017, "PolyEditTool vertex_renderer may only reference XYGlyph models")
NO_RANGE_TOOL_RANGES                               = Error(1018, "RangeTool must have at least one of x_range or y_range configured")
DUPLICATE_FACTORS                                  = Error(1019, "FactorRange must specify a unique list of categorical factors for an axis")
BAD_EXTRA_RANGE_NAME                               = Error(1020, "An extra range name is configued with a name that does not correspond to any range")
EQUAL_SLIDER_START_END                             = Error(1021, "Slider 'start' and 'end' cannot be equal")
MIN_PREFERRED_MAX_WIDTH                            = Error(1022, "Expected min_width <= width <= max_width")
MIN_PREFERRED_MAX_HEIGHT                           = Error(1023, "Expected min_height <= height <= max_height")
CDSVIEW_FILTERS_WITH_CONNECTED                     = Error(1024, "CDSView filters are not compatible with glyphs with connected topology such as Line or Patch")
INCOMPATIBLE_LINE_EDIT_RENDERER                    = Error(1025, "LineEditTool renderers may only reference MultiLine and Line glyph models")
INCOMPATIBLE_LINE_EDIT_INTERSECTION_RENDERER       = Error(1026, "LineEditTool intersection_renderer may only reference LineGlyph models")
REPEATED_LAYOUT_CHILD                              = Error(1027, "The same model can't be used multiple times in a layout")
EXT                                                = Error(9999, "Custom extension reports error")
