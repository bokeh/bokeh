...
class TestBar(unittest.TestCase):
    def test_supported_input(self):
        ...
        for i, _xy in enumerate([xyvalues,
            ...
            pre = builder.prefix
            self.assertEqual(pre, 'bar_%s_' % (bar._id.lower().replace("-", "_")))
            ...
            np.testing.assert_array_equal(builder._data[pre + 'midpython'], np.array([1, 2.5]))
            np.testing.assert_array_equal(builder._data[pre + 'midpypy'], np.array([6, 20]))
            np.testing.assert_array_equal(builder._data[pre + 'midjython'], np.array([11, 15]))
            ...
            if i != 1:
                np.testing.assert_array_equal(builder._data[pre + 'stackedpython'], np.array([1, 2.5]))
                np.testing.assert_array_equal(builder._data[pre + 'stackedpypy'], np.array([8, 25]))
                np.testing.assert_array_equal(builder._data[pre + 'stackedjython'], np.array([25, 60]))

            np.testing.assert_array_equal(builder._data[pre + 'cat'], np.array(['0', '1']))
            np.testing.assert_array_equal(builder._data[pre + 'width'], np.array([0.8, 0.8]))
            np.testing.assert_array_equal(builder._data[pre + 'width_cat'], np.array([0.2, 0.2]))
        ...
        for i, _xy in enumerate([lvalues, np.array(lvalues)]):
            ...
            # test mid values, that should always be y/2 ..
            np.testing.assert_array_equal(builder._data[pre + 'mid0'], np.array([1, 2.5]))
            np.testing.assert_array_equal(builder._data[pre + 'mid1'], np.array([6, 20]))
            np.testing.assert_array_equal(builder._data[pre + 'mid2'], np.array([11, 15]))

            # stacked values should be 0 as base and + y/2 of the column
            np.testing.assert_array_equal(builder._data[pre + 'stacked0'], np.array([1, 2.5]))
            np.testing.assert_array_equal(builder._data[pre + 'stacked1'], np.array([8, 25]))
            np.testing.assert_array_equal(builder._data[pre + 'stacked2'], np.array([25, 60]))
