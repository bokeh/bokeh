...
test('renderer_selection', ->
  ...
  scatterrenderer.select([0, 200], null, {'local' : true})
  deepEqual(scatterrenderer.get_ref('data_source').get('selected'), _.range(5))
  scatterrenderer.unselect({'local' : true})
  scatterrenderer.select(null, [0, 200], {'local' : true})
  deepEqual(scatterrenderer.get_ref('data_source').get('selected'), _.range(5))
  scatterrenderer.unselect({'local' : true})
  scatterrenderer.select([0, 120], null, {'local' : true})
  deepEqual(scatterrenderer.get_ref('data_source').get('selected'), _.range(3))
  scatterrenderer.unselect({'local' : true})
  scatterrenderer.select([0, 120], [80, 200], {'local' : true})
  deepEqual(scatterrenderer.get_ref('data_source').get('selected'), _.range(3))
