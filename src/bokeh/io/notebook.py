...
def remote_jupyter_proxy_url(port: int):
    ...
    and made an implicit override when BOKEH_JUPYTER_EXTERNAL_URL is defined in
    ...
    Args:
       ...
       Returns: f(port: int) -> str
           a callable which given a random port generates an external url which
           will return through the proxy to the current notebook server.
    """
    base_url = os.environ['BOKEH_JUPYTER_EXTERNAL_URL']
