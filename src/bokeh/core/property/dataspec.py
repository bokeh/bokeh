...
class DataSpec(Either[Any]):
    ...
    When these records are received in the browser, BokehJS knows how to
    interpret them and take the correct,
    ...
    Use the :func:`~bokeh.core.properties.value` and
    :func:`~bokeh.core.properties.field` helpers when the interpretation must
    be explicit:
    ...
    .. code-block:: python
        ...
        glyph.x = field("pressure")

    A ``DataSpec`` can have ``units`` and ``transform`` modifiers. The latter
    specifies a client-side transform applied before materialization. For
    example:

    .. code-block:: python

        glyph.x = value(10, Jitter(width=0.4))
...
class ColorSpec(DataSpec):
    ...
    This automatic interpretation can be overridden with the |field| helper:
