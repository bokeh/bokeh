...
``render_mode`` was removed
~~~~~~~~~~~~~~~~~~~~~~~~~~~

Annotations were cleanly separated between those which render to canvas and
those that use HTML and CSS. The default is canvas rendering. Use the new
``bokeh.models.annotations.html`` models whenever previously ``css`` render
mode was used. Note that ``Span`` and ``BoxAnnotation`` don't have a HTML
equivalent.
