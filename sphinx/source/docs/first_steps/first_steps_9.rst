...
Widgets are interactive control elements that you can add to your Bokeh
documents.

See :ref:`userguide_interaction_widgets_examples` in the user guide for a
complete list of examples.

You can use widgets to control various aspects of your visualization directly in
the browser. For example:

* Use a :class:`~bokeh.models.widgets.markups.Div` widget to display HTML text
* Use a :class:`~bokeh.models.widgets.inputs.Spinner` widget to select a numeric
  value
* use a :class:`~bokeh.models.widgets.sliders.RangeSlider` widget to adjust a
  range

.. bokeh-plot:: docs/first_steps/examples/first_steps_9_widgets.py
    :source-position: none

Follow those steps to add those widgets to your code:

1. Import the ``ColorPicker``, ``Div``, ``RangeSlider``, and ``Spinner`` widgets
   from ``bokeh.models``:

    .. code-block:: python

        from bokeh.models import ColorPicker, Div, RangeSlider, Spinner
...
6. Create a RangeSlider object:
    ...
    .. code-block:: python
        ...
        range_slider = RangeSlider(
            start=0, end=10,  # set the maximum and minimum values for the slider
...
7. To link the values generated by the RangeSlider to the existing plot, use the
   ...
   want to assign two values at once: the beginning of the plot's x-axis and
...
This is what the completed example looks like, with the widget-specific portions
of the code highlighted:

.. literalinclude:: examples/first_steps_9_widgets.py
   :language: python
   :emphasize-lines: 2,17-20,23-31,34-40
