...
task("scripts:bundle", [passthrough("scripts:compile")], async () => {
  const {bokehjs, api, widgets, tables, mathjax} = paths.lib
  const packages = [bokehjs, api, widgets, tables, mathjax]
...
task("scripts:bundle-legacy", [passthrough("scripts:compile")], async () => {
  const {bokehjs, api, widgets, tables, mathjax} = paths.lib_legacy
  const packages = [bokehjs, api, widgets, tables, mathjax]
