...
import {TableDataSource} from "../sources/table_data_source"
...
export class GlyphRendererView extends RendererView
  ...
  bind_bokeh_events: () ->
    ...
    if @model.data_source.cds?
      @listenTo(@model.data_source.cds, 'select', @request_render)
    else
  ...
  render: () ->
    ...
    if @model.data_source.cds?
      selected = @model.data_source.cds.selected
    else
