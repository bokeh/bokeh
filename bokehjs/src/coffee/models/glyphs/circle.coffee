...
export class CircleView extends XYGlyphView
  ...
  _mask_data: (all_indices) ->
    ...
    if @_radius? and @model.properties.radius.units == "data"
      sx0 = hr.start
      sx1 = hr.end
      [x0, x1] = @renderer.xmapper.v_invert([sx0, sx1], true)
      x0 -= @max_radius
      x1 += @max_radius

      sy0 = vr.start
      sy1 = vr.end
      [y0, y1] = @renderer.ymapper.v_invert([sy0, sy1], true)
      y0 -= @max_radius
      y1 += @max_radius
  ...
  _hit_span: (geometry) ->
      ...
      if geometry.direction == 'h'
        ...
        if @_radius? and @model.properties.radius.units == "data"
          vx0 = vx - @max_radius
          vx1 = vx + @max_radius
          [x0, x1] = @renderer.xmapper.v_invert([vx0, vx1])
        else
          ms = @max_size/2
          vx0 = vx - ms
          vx1 = vx + ms
        ...
        if @_radius? and @model.properties.radius.units == "data"
          vy0 = vy - @max_radius
          vy1 = vy + @max_radius
          [y0, y1] = @renderer.ymapper.v_invert([vy0, vy1])
        else
          ms = @max_size/2
          vy0 = vy - ms
          vy1 = vy + ms
