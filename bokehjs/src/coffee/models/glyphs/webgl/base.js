...
export class BaseGLGlyph {
  ...
  render(ctx, indices, mainglyph) {
    ...
    let [dx, dy] = Array.from(this.glyph.renderer.map_to_screen([0*wx, 1*wx, 2*wx], [0*wy, 1*wy, 2*wy]));
    ...
    [dx, dy] = Array.from(this.glyph.renderer.map_to_screen([0*wx, 1*wx, 2*wx], [0*wy, 1*wy, 2*wy]));
    // Test how linear it is
    if ((Math.abs((dx[1] - dx[0]) - (dx[2] - dx[1])) > 1e-6) ||
        (Math.abs((dy[1] - dy[0]) - (dy[2] - dy[1])) > 1e-6)) {
      return false;
    }
    const [sx, sy] = Array.from([(dx[1]-dx[0]) / wx, (dy[1]-dy[0]) / wy]);
    const trans = {
