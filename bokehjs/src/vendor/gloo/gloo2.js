...
gloo2 = (function () {
    ...
    Program.prototype.set_texture = function (name, value) {
        var dummy15_, dummy16_, dummy17_, err_3, handle, unit;
        ...
        handle = (this._handles[name] || (-1));
        if (handle < 0) {
            if (((dummy15_ = this._known_invalid).indexOf ? dummy15_ : Object.keys(dummy15_)).indexOf(name) >= 0) {
                return;
            }
            handle = this._gl.getUniformLocation(this._handle, name);
            if (((dummy16_ = this._unset_variables).indexOf ? dummy16_ : Object.keys(dummy16_)).indexOf(name) >= 0) {
        ...
        if (true) {
            ...
            if (((dummy17_ = this._samplers).indexOf ? dummy17_ : Object.keys(dummy17_)).indexOf(name) >= 0) {
    ...
    Program.prototype.set_uniform = function (name, type_, value) {
        var count, dummy18_, dummy19_, dummy20_, dummy21_, err_3, funcname, handle, ii, name_;
        ...
        count = 1;
        if (handle < 0) {
            if (((dummy18_ = this._known_invalid).indexOf ? dummy18_ : Object.keys(dummy18_)).indexOf(name) >= 0) {
                return;
            }
            handle = this._gl.getUniformLocation(this._handle, name);
            if (((dummy19_ = this._unset_variables).indexOf ? dummy19_ : Object.keys(dummy19_)).indexOf(name) >= 0) {
                (this._unset_variables.remove || function (x) {this.splice(this.indexOf(x), 1);}).apply(this._unset_variables, [name]);
            }
            ...
            if (count > 1) {
                for (ii = 0; ii < count; ii += 1) {
                    if (((dummy20_ = this._unset_variables).indexOf ? dummy20_ : Object.keys(dummy20_)).indexOf(('' + name + '[' + ii + ']')) >= 0) {
                        name_ = '' + name + '[' + ii + ']';
                        if (((dummy21_ = this._unset_variables).indexOf ? dummy21_ : Object.keys(dummy21_)).indexOf(name_) >= 0) {
    ...
    Program.prototype.set_attribute = function (name, type_, value) {
        var args, dummy22_, dummy23_, dummy24_, dummy25_, err_3, funcname, gtype, handle, offset, size, stride, vbo;
        ...
        handle = (this._handles[name] || (-1));
        if (handle < 0) {
            if (((dummy22_ = this._known_invalid).indexOf ? dummy22_ : Object.keys(dummy22_)).indexOf(name) >= 0) {
                return;
            }
            handle = this._gl.getAttribLocation(this._handle, name);
            if (((dummy23_ = this._unset_variables).indexOf ? dummy23_ : Object.keys(dummy23_)).indexOf(name) >= 0) {
                (this._unset_variables.remove || function (x) {this.splice(this.indexOf(x), 1);}).apply(this._unset_variables, [name]);
            }
            this._handles[name] = handle;
            if (handle < 0) {
        ...
        if ((value[0]) == 0) {
            ...
            dummy24_ = value;
            vbo = dummy24_[0];stride = dummy24_[1];offset = dummy24_[2];
            dummy25_ = this.ATYPEINFO[type_];
            size = dummy25_[0];gtype = dummy25_[1];
    ...
    Program.prototype._pre_draw = function () {
        var args, attr_handle, dummy26_, dummy27_sequence, dummy28_, dummy29_sequence, funcname, tex_handle, tex_target, unit, vbo_handle, x;
        this.activate();
        dummy27_sequence = this._samplers;
        for (x in dummy27_sequence) {
            if (!dummy27_sequence.hasOwnProperty(x)){ continue; }
            x = dummy27_sequence[x];
            dummy26_ = x;
            tex_target = dummy26_[0];tex_handle = dummy26_[1];unit = dummy26_[2];
            this._gl.activeTexture(this._gl.TEXTURE0 + unit);
            this._gl.bindTexture(tex_target, tex_handle);
        }
        dummy29_sequence = this._attributes;
        for (x in dummy29_sequence) {
            if (!dummy29_sequence.hasOwnProperty(x)){ continue; }
            x = dummy29_sequence[x];
            dummy28_ = x;
            vbo_handle = dummy28_[0];attr_handle = dummy28_[1];funcname = dummy28_[2];args = dummy28_[3];
            if (vbo_handle) {
    ...
    Program.prototype.draw = function (mode, selection) {
        var count, dummy30_, dummy31_, e, err_3, first, gtype, ibuf, id_;
        ...
        if (selection.length == 3) {
            dummy30_ = selection;
            id_ = dummy30_[0];gtype = dummy30_[1];count = dummy30_[2];
            if (count) {
                this._pre_draw();
                ibuf = (this._gl._objects[id_] || null);
                ibuf.activate();
            ...
            dummy31_ = selection;
            first = dummy31_[0];count = dummy31_[1];
            if (count) {
                this._pre_draw();
    ...
    Texture2D.prototype._get_alignment = function (width) {
        var alignment, alignments, dummy32_sequence, dummy33_iter, dummy34_length;
        ...
        alignments = [4, 8, 2, 1];
        dummy32_sequence = alignments;
        if ((typeof dummy32_sequence === "object") && (!Array.isArray(dummy32_sequence))) {
            dummy32_sequence = Object.keys(dummy32_sequence);
        }
        dummy34_length = dummy32_sequence.length;
        for (dummy33_iter = 0; dummy33_iter < dummy34_length; dummy33_iter += 1) {
            alignment = dummy32_sequence[dummy33_iter];
    ...
    Texture2D.prototype.set_size = function (shape, format) {
        var dummy35_, height, width;
        dummy35_ = shape;
        height = dummy35_[0];width = dummy35_[1];
    ...
    Texture2D.prototype.set_data = function (offset, shape, data) {
        var dummy36_, dummy37_, err_3, format, gtype, height, width, x, y;
        this.activate();
        format = this._shape_format[2];
        dummy36_ = shape;
        height = dummy36_[0];width = dummy36_[1];
        dummy37_ = offset;
        y = dummy37_[0];x = dummy37_[1];
        gtype = (this._types[data.constructor.name] || null);
        if (gtype === null) {
    ...
    /* ===== CREATING MODULE ===== */
    // note that "this" is the global object
    var g = {}
    /*
    try {
        g = global;
    ...
    */
    if (g.gloo2 === undefined) {
        g.gloo2 = {};
    }
    
    g.gloo2.Buffer = Buffer;
    g.gloo2.GlooObject = GlooObject;
    g.gloo2.IndexBuffer = IndexBuffer;
    g.gloo2.Program = Program;
    g.gloo2.Texture2D = Texture2D;
    g.gloo2.VertexBuffer = VertexBuffer;
    g.gloo2.check_error = check_error;
    return g.gloo2;
})();
...
module.exports = {
    'Buffer': gloo2.Buffer,
    'GlooObject': gloo2.GlooObject,
    'IndexBuffer': gloo2.IndexBuffer,
    'Program': gloo2.Program,
    'Texture2D': gloo2.Texture2D,
    'VertexBuffer': gloo2.VertexBuffer,
    'check_error': gloo2.check_error
};
