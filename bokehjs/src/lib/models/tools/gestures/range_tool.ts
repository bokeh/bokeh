import {BoxAnnotation, Directions, Edges} from "../../annotations/box_annotation"
...
export class RangeToolView extends GestureToolView {
  ...
  override connect_signals(): void {
    ...
    this.model.overlay.pan.connect((state) => {
      if (state == "pan") {
...
export class RangeTool extends GestureTool {
  ...
  override initialize(): void {
    ...
    this.overlay.movable = (() => {
      const x = has_x ? Directions.X : Directions.None
      const y = has_y ? Directions.Y : Directions.None
      return x | y
    })()
    this.overlay.resizable = (() => {
      const x = has_x ? Edges.X : Edges.None
      const y = has_y ? Edges.Y : Edges.None
      return x | y
    })()
  }
