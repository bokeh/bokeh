...
export class EllipseView extends XYGlyphView {
  ...
  protected _point_in_ellipse(x0: number, y0: number, angle: number, h: number, w: number, x: number, y: number){
  ...
  protected _hit_point(geometry: PointGeometry): Selection {
    ...
    for (const i of candidates) {
      cond = this._point_in_ellipse(sx, sy, this._angle[i], this.sh[i]/2, this.sw[i]/2, this.sx[i], this.sy[i])
      if (cond){
