...
export type ExecutableLike<Obj, Args extends unknown[], Ret = void> = Executable<Obj, Args, Ret> | Callable<Obj, Args, Ret>
export type ExecutableOf<T extends ExecutableLike<any, any, any>> = T extends Function ? never : T

export function execute<Obj, Args extends unknown[], Ret>(cb: ExecutableLike<Obj, Args, Ret>, obj: Obj, ...args: Args): Ret | Promise<Ret> {
  if (isFunction(cb)) {
    return cb(obj, ...args)
  } else {
...
export type CallbackLike<Obj, Args extends unknown[], Ret = void> = ExecutableLike<Obj, Args, Ret>
