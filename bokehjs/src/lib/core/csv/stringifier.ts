...
export class Stringifier {
  ...
  stringify(records: Iterable<CSVRecord>): string {
    ...
    for (const record of records) {
      ...
      result = result + this.stringify_record(record, context) + "\n"
      row_index++
    }
    return result;
  }
  ...
  // Any quotes inside a CSV field must be escaped with a quote. Important:
  // because this function looks for quotes, it must be called before the quote
  // function.
