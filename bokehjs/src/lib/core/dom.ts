...
type WritablePartOf<T> = Pick<T, WritableKeysOf<T>>

type Optional<T> = {[P in keyof T]?: T[P] | null | undefined}

type HTMLElementName = keyof HTMLElementTagNameMap

type HTMLElementExtras<T extends HTMLElementName> = T extends "label" ? {for: string} : {}

type CSSClass = string

export type HTMLAttrs<T extends HTMLElementName> = Optional<WritablePartOf<T>> & Optional<HTMLElementExtras<T>> & {
...
const _element = <T extends keyof HTMLElementTagNameMap>(tag: T) => {
  return (attrs: HTMLAttrs<T> | HTMLChild = {}, ...children: HTMLChild[]): HTMLElementTagNameMap[T] => {
...
/** @deprecated */
export const createElement = create_element
