...
export namespace Anscombe {
  ...
  function make_plot(title: string, xname: string, yname: string): Bokeh.Plot {
    ...
    const x_axis = new Bokeh.LinearAxis({axis_line_color: null})
    const y_axis = new Bokeh.LinearAxis({axis_line_color: null})
    plot.add_layout(x_axis, "below")
    plot.add_layout(y_axis, "left")
    const x_grid = new Bokeh.Grid({ticker: x_axis.ticker, dimension: 0})
    const y_grid = new Bokeh.Grid({ticker: y_axis.ticker, dimension: 1})
