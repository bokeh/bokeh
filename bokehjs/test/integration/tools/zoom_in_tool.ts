...
import {Range1d, FactorRange, ZoomInTool, LinearScale, CategoricalScale} from "@bokehjs/models"
...
describe("ZoomInTool", () => {
  it("should support zooming sub-coordinates", async () => {
    function plot(title: string) {
      ...
      const y_range = FactorRange.create({factors: ["A", "B", "C"]})
    ...
    const zoom_in1 = ZoomInTool.create({renderers: data_renderers(p1), level: 0})
    p1.add_tools(zoom_in1)

    const zoom_in2 = ZoomInTool.create({renderers: data_renderers(p2), level: 1})
