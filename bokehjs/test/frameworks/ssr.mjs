...
const plot = Bokeh.Plotting.figure({width: 240, height: 160, tools: ["xpan", "reset"]})
plot.line([0, 1], [1, 0])
assert.equal(plot.width, 240)
assert.ok(plot.toolbar.tools[0] instanceof Bokeh.PanTool)
assert.equal(plot.toolbar.tools[0].dimensions, "width")
assert.ok(plot.toolbar.tools[1] instanceof Bokeh.ResetTool)
