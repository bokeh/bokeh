...
describe("events module", () => {
  ...
  describe("ColumnsPatchedEvent", () => {
    it("should generate json", () => {
      ...
      const event = new events.ColumnsPatchedEvent(d, m, {foo: [[1, 2]]})
    ...
    it("should support equality", () => {
      ...
      const m = new ColumnDataSource()
      const ev0 = new events.ColumnsPatchedEvent(doc, m, {column: [[0, "a"]]})
      const ev1 = new events.ColumnsPatchedEvent(doc, m, {column: [[0, "b"]]})
  ...
  describe("ColumnsStreamedEvent", () => {
    it("should generate json with rollover", () => {
      const d = new Document()
      const m = new ColumnDataSource()
      const event = new events.ColumnsStreamedEvent(d, m, {foo: [1, 2], bar: [3, 4]}, 10)
      const serializer = new Serializer()
      const result = serializer.to_serializable(event)
      expect(result).to.be.equal({
    ...
    it("should generate json without rollover", () => {
      const d = new Document()
      const m = new ColumnDataSource()
      const event = new events.ColumnsStreamedEvent(d, m, {foo: [1, 2], bar: [3, 4]})
      const serializer = new Serializer()
      const result = serializer.to_serializable(event)
      expect(result).to.be.equal({
        kind: "ColumnsStreamed",
    ...
    it("should support equality", () => {
      const doc = new Document()
      const m = new ColumnDataSource()
      const ev0 = new events.ColumnsStreamedEvent(doc, m, {x: [1]})
      const ev1 = new events.ColumnsStreamedEvent(doc, m, {x: [2]})
      expect(is_equal(ev0, ev0)).to.be.equal(true)
      expect(is_equal(ev0, ev1)).to.be.equal(false)
    })
  })
