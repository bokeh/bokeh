...
describe("linear_scale module", () => {
  ...
  describe("creation with Range1d ranges", () => {
    ...
    it("should vector map values linearly", () => {
      expect(scale.v_compute([-1, 0, 5, 10, 11])).to.be.equal(new Float32Array([14, 20, 50, 80, 86]))
    })
    ...
    it("should vector in inverse map values linearly", () => {
      expect(scale.v_invert([14, 20, 50, 80, 86])).to.be.equal(new Float32Array([-1, 0, 5, 10, 11]))
    })

    it("should inverse map to a Float32Array", () => {
      expect(scale.v_invert([-1, 0, 5, 10, 11])).to.be.instanceof(Float32Array)
    })
  })
})
