...
describe("BoxZoomTool", () => {
  ...
  describe("Model", () => {
    ...
    it("should create proper tooltip", () => {
      const tool = BoxZoomTool.create()
      expect(tool.tooltip).to.be.equal("Box Zoom (either x, y or both dimensions)")

      const x_tool = BoxZoomTool.create({dimensions: "width"})
      expect(x_tool.tooltip).to.be.equal("Box Zoom (x-axis)")

      const y_tool = BoxZoomTool.create({dimensions: "height"})
      expect(y_tool.tooltip).to.be.equal("Box Zoom (y-axis)")

      const tool_custom = BoxZoomTool.create({description: "My box zoom tool"})
      expect(tool_custom.tooltip).to.be.equal("My box zoom tool")

      const x_tool_custom = BoxZoomTool.create({dimensions: "width", description: "My box x-zoom tool"})
      expect(x_tool_custom.tooltip).to.be.equal("My box x-zoom tool")

      const y_tool_custom = BoxZoomTool.create({dimensions: "height", description: "My box y-zoom tool"})
  ...
  describe("View", () => {
    async function mkplot(...tools: Tool[]) {
      ...
      const plot = Plot.create({
        ...
        toolbar: Toolbar.create({buttons, tools}),
      })
    ...
    it("test_deselected_by_default_with_pan_tool", async () => {
      const {buttons} = await mkplot(BoxZoomTool.create(), PanTool.create())
    ...
    it("selected_by_default_without_pan_tool", async () => {
      const {buttons} = await mkplot(BoxZoomTool.create())
      const [zoom] = buttons
      expect(zoom.class_list.has("bk-active")).to.be.true
    })

    it("can_be_selected_and_deselected", async () => {
      const {buttons, plot_view} = await mkplot(BoxZoomTool.create(), PanTool.create())
      const [zoom, pan] = buttons
    ...
    it("should zoom in both ranges", async () => {
      const box_zoom = BoxZoomTool.create()
    ...
    it("should zoom in with match_aspect", async () => {
      const box_zoom = BoxZoomTool.create({match_aspect: true})
      const {plot_view} = await mkplot(box_zoom)
    ...
    it("should show correct zoom overlay with both dimensions", async () => {
      const box_zoom = BoxZoomTool.create({dimensions: "both"})
      const {plot_view} = await mkplot(box_zoom)
    ...
    it("should show correct zoom overlay with width dimensions", async () => {
      const box_zoom = BoxZoomTool.create({dimensions: "width"})
      const {plot_view} = await mkplot(box_zoom)
    ...
    it("should show correct zoom overlay with height dimensions", async () => {
      const box_zoom = BoxZoomTool.create({dimensions: "height"})
      const {plot_view} = await mkplot(box_zoom)
