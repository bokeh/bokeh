...
describe("Canvas", () => {
  describe("should support composing layers", () => {
    it.dpr(1)("with devicePixelRatio == 1", async () => {
      const canvas = Canvas.create({output_backend: "canvas", styles: {width: "600px", height: "600px"}})
    ...
    it("with SVG backend", async () => {
      const canvas = Canvas.create({output_backend: "svg", styles: {width: "600px", height: "600px"}})
      const {view: canvas_view} = await display(canvas, [650, 650])
