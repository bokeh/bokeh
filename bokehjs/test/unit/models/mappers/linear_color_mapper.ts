...
describe("LinearColorMapper module", () => {
  ...
  describe("LinearColorMapper.v_compute method", () => {
    ...
    it("Should map values along linear scale with high/low unset", () => {
      ...
      expect(vals0).to.be.equal(_convert_palette(["blue", "blue", "green", "red", "red"]))

      const vals1 = color_mapper.v_compute([0.0001, 32, 50, 67, 99.999])
      expect(vals1).to.be.equal(_convert_palette(["red", "red", "green", "blue", "blue"]))

      const vals2 = color_mapper.v_compute([1, 2, 3])
      expect(vals2).to.be.equal(_convert_palette(["red", "green", "blue"]))

      const vals3 = color_mapper.v_compute([3, 2, 1])
      expect(vals3).to.be.equal(_convert_palette(["blue", "green", "red"]))

      const vals4 = color_mapper.v_compute([0, 1, 2])
      expect(vals4).to.be.equal(_convert_palette(["red", "green", "blue"]))

      const vals5 = color_mapper.v_compute([2, 1, 0])
      expect(vals5).to.be.equal(_convert_palette(["blue", "green", "red"]))

      const vals6 = color_mapper.v_compute([-1, 0, 1])
      expect(vals6).to.be.equal(_convert_palette(["red", "green", "blue"]))

      const vals7 = color_mapper.v_compute([1, 0, -1])
      expect(vals7).to.be.equal(_convert_palette(["blue", "green", "red"]))
    })
    ...
    it("Should map values along linear scale with high/low set", () => {
      ...
      expect(vals).to.be.equal(_convert_palette(["red", "green", "blue"]))
    })
    ...
    it("Should map values along linear scale with high/low set in other direction", () => {
      ...
      const vals = color_mapper.v_compute([1, 2, 3])
      expect(vals).to.be.equal(_convert_palette(["blue", "green", "red"]))
    })
    ...
    it("Should map data below low value to low", () => {
      ...
      expect(vals).to.be.equal(_convert_palette(["red", "red", "green"]))
    })
    ...
    it("Should map data above high value to high", () => {
      ...
      expect(vals).to.be.equal(_convert_palette(["green", "blue", "blue"]))
    })
    ...
    it("Should map data NaN to nan_color value", () => {
      ...
      expect(vals).to.be.equal(_convert_palette(["red", "gray", "blue"]))
    })
    ...
    it("Should map high/low values to high_color/low_color, if provided", () => {
      ...
      expect(vals).to.be.equal(_convert_palette(["pink", "red", "green", "blue", "orange"]))
    })
  })
})
