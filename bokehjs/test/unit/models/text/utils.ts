...
import {TeX, TranslatableText} from "@bokehjs/models/text"
...
describe("models/text/utils module", () => {
  it("should provide parse_delimited_string() function", () => {
    ...
    expect(s3).to.be.structurally.equal(new TranslatableText({text: "test$$"}))

    const s4 = parse_delimited_string("$$test")
    expect(s4).to.be.structurally.equal(new TranslatableText({text: "$$test"}))

    const s5 = parse_delimited_string("test\\]")
    expect(s5).to.be.structurally.equal(new TranslatableText({text: "test\\]"}))

    const s6 = parse_delimited_string("\\[test")
    expect(s6).to.be.structurally.equal(new TranslatableText({text: "\\[test"}))

    const s7 = parse_delimited_string("test\\)")
    expect(s7).to.be.structurally.equal(new TranslatableText({text: "test\\)"}))

    const s8 = parse_delimited_string("\\(test")
    expect(s8).to.be.structurally.equal(new TranslatableText({text: "\\(test"}))
    ...
    expect(s11).to.be.structurally.equal(new TranslatableText({text: "$$test\\]"}))
