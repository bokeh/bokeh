...
describe("CustomJSTicker Model", () => {
  ...
  describe("Continuous get_ticks method", () => {
    it("should handle case with no major_code", () => {
      const ticker = CustomJSTicker.create()
      const range = Range1d.create({start: -1, end: 11})
    ...
    it("should return major_code result", () => {
      const ticker = CustomJSTicker.create({major_code: "return [2,4,6,8]"})
      const range = Range1d.create({start: -1, end: 11})
      const ticks = ticker.get_ticks(0, 10, range, NaN)
    ...
    it("should pass start and end to major_code", () => {
      const ticker = CustomJSTicker.create({major_code: "return [cb_data.start, cb_data.end]"})
      const range = Range1d.create({start: -1, end: 11})
      const ticks = ticker.get_ticks(0, 10, range, NaN)
    ...
    it("should pass range to major_code", () => {
      const ticker = CustomJSTicker.create({major_code: "return [cb_data.range.start, cb_data.range.end]"})
      const range = Range1d.create({start: -1, end: 11})
      const ticks = ticker.get_ticks(0, 10, range, NaN)
    ...
    it("should pass cross_loc to major_code", () => {
      const ticker = CustomJSTicker.create({major_code: "return [cb_data.cross_loc]"})
      const range = Range1d.create({start: -1, end: 11})
  ...
  describe("Categorical get_ticks method", () => {
    ...
    it("should handle case with no major_code", () => {
      const ticker = CustomJSTicker.create()
      const range = FactorRange.create({factors: ["foo", "bar", "baz"]})
    ...
    it("should handle case where range has factors", () => {
      const ticker = CustomJSTicker.create({major_code: "return['foo', 'baz']"})
      const range = FactorRange.create({factors: ["foo", "bar", "baz"]})
    ...
    it("should pass start and end to major_code", () => {
      const ticker = CustomJSTicker.create({major_code: "return [cb_data.start.toString(), cb_data.end.toString()]"})
      const range = FactorRange.create({factors: ["foo", "bar", "baz"]})
      const ticks = ticker.get_ticks(0, 10, range, NaN) as FactorTickSpec
      expect(ticks.major).to.be.equal(["0", "10"])
      expect(ticks.minor).to.be.equal([])
    ...
    it("should pass range to major_code", () => {
      const ticker = CustomJSTicker.create({major_code: "return cb_data.range.factors"})
      const range = FactorRange.create({factors: ["foo", "bar", "baz"]})
      const ticks = ticker.get_ticks(0, 10, range, NaN) as FactorTickSpec
    ...
    it("should pass cross_loc to major_code", () => {
      const ticker = CustomJSTicker.create({major_code: "return [cb_data.cross_loc.toString()]"})
      const range = FactorRange.create({factors: ["foo", "bar", "baz"]})
