...
describe("Stack", () => {
  ...
  it("should should compute for a source", () => {
    ...
    expect(ret0).to.be.deep.equal(new Float64Array([1, 2, 3]))

    const s1 = new Stack({fields: ['foo', 'bar']})
    const ret1 = s1.v_compute(source)
    expect(ret1).to.be.deep.equal(new Float64Array([1.1, 2.2, 3.3]))
  })
  ...
  it("should should compute for different sources", () => {
    ...
    expect(ret2).to.be.deep.equal(new Float64Array([10, 20, 30]))

    const s3 = new Stack({fields: ['foo', 'bar']})
    const ret3 = s3.v_compute(source2)
    expect(ret3).to.be.deep.equal(new Float64Array([10.01, 20.02, 30.03]))
  })
  ...
  it("should should re-compute if a source changes", () => {
    ...
    expect(ret0).to.be.deep.equal(new Float64Array([1.1, 2.2, 3.3]))

    source.data = {foo: [10, 20, 30], bar: [0.01, 0.02, 0.03]}
    const ret1 = s.v_compute(source)
    expect(ret1).to.be.deep.equal(new Float64Array([10.01, 20.02, 30.03]))
  })
  ...
  it("should should re-compute if a source patches", () => {
    ...
    const ret1 = s.v_compute(source)
    expect(ret1).to.be.deep.equal(new Float64Array([1.1, 12.2, 3.3]))

    source.patch({bar: [[0, 1.1]]})
    const ret2 = s.v_compute(source)
    expect(ret2).to.be.deep.equal(new Float64Array([2.1, 12.2, 3.3]))
  })
  ...
  it("should should re-compute if a source streams", () => {
    ...
    const ret1 = s.v_compute(source)
    expect(ret1).to.be.deep.equal(new Float64Array([1.1, 2.2, 3.3, 4.4]))
  })
})
