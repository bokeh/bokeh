  ...
  describe("should support log axes", () => {
    async function test(side: Side, pt: Point) {
      const plot = Plot.create({
        ...
        title: null,
        toolbar_location: null,
        x_range: Range1d.create({start: 1, end: 10000}),
        y_range: Range1d.create({start: 1, end: 10000}),
      })
      const axis = LogAxis.create()
      plot.add_layout(axis, side)
  ...
  describe("should support categorical axes", () => {
    async function test(side: Side, pt: Point) {
      const plot = Plot.create({
        ...
        x_range: FactorRange.create({factors: ["a", "b", "c", "d", "e"]}),
        y_range: FactorRange.create({factors: ["a", "b", "c", "d", "e"]}),
      })
      const axis = CategoricalAxis.create()
      plot.add_layout(axis, side)
