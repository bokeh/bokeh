...
import {expr, field, isExpr, value} from "@bokehjs/core/vectorization"
...
describe("properties module", () => {
  ...
  describe("DataSpec dependencies", () => {
    it("should notify through ordinary property and model change signals", () => {
      const transform = new TestTransform()
      const expression = new TestExpression()
      const obj0 = new Some({number_spec: field("foo", transform)})
      const obj1 = new Some({number_spec: expr(expression)})
      let property_changes = 0
      let model_changes = 0
      obj0.properties.number_spec.change.connect(() => property_changes++)
      obj0.change.connect(() => model_changes++)
      obj1.properties.number_spec.change.connect(() => property_changes++)
      obj1.change.connect(() => model_changes++)

      transform.name = "updated"
      expression.name = "updated"

      expect(property_changes).to.be.equal(2)
      expect(model_changes).to.be.equal(2)
    })

    it("should reconnect when a transform is replaced", () => {
      const transform0 = new TestTransform()
      const transform1 = new TestTransform()
      const obj = new Some({number_spec: field("foo", transform0)})
      let changes = 0
      obj.change.connect(() => changes++)

      obj.properties.number_spec.get_value().transform = transform1
      changes = 0
      transform0.name = "stale"
      expect(changes).to.be.equal(0)
      transform1.name = "current"
      expect(changes).to.be.equal(1)
    })

    it("should reconnect when an expression is replaced", () => {
      const expression0 = new TestExpression()
      const expression1 = new TestExpression()
      const obj = new Some({number_spec: expr(expression0)})
      let changes = 0
      obj.change.connect(() => changes++)

      const spec = obj.properties.number_spec.get_value()
      if (!isExpr(spec)) {
        throw new Error("expected an expression spec")
      }
      spec.value = expression1
      changes = 0
      expression0.name = "stale"
      expect(changes).to.be.equal(0)
      expression1.name = "current"
      expect(changes).to.be.equal(1)
    })
  })
