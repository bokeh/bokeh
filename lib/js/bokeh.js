(function() {
  var Bokeh, BokehView, Collections, Component, D3LinearAxes, D3LinearAxis, D3LinearAxisView, DiscreteColorMapper, DiscreteColorMappers, FactorRange, FactorRanges, GraphicsContext, GraphicsContextView, GraphicsContexts, HasParent, HasProperties, HasReference, LinearMapper, LinearMappers, Mapper, ObjectArrayDataSource, ObjectArrayDataSources, Plot, PlotView, Plots, Range1d, Range1ds, Renderer, ScatterRenderer, ScatterRendererView, ScatterRenderers,
  ...
  BokehView = (function(_super) {
    ...
    BokehView.prototype.tag_selector = function(tag, id) {
  ...
  GraphicsContext = (function(_super) {

    __extends(GraphicsContext, _super);

    function GraphicsContext() {
      GraphicsContext.__super__.constructor.apply(this, arguments);
    }

    GraphicsContext.prototype.parent_properties = ['width', 'height'];

    return GraphicsContext;

  })(Component);

  GraphicsContexts = (function(_super) {

    __extends(GraphicsContexts, _super);

    function GraphicsContexts() {
      GraphicsContexts.__super__.constructor.apply(this, arguments);
    }

    GraphicsContexts.prototype.model = GraphicsContext;

    return GraphicsContexts;

  })(Backbone.Collection);

  GraphicsContextView = (function(_super) {

    __extends(GraphicsContextView, _super);

    function GraphicsContextView() {
      GraphicsContextView.__super__.constructor.apply(this, arguments);
    }

    GraphicsContextView.prototype.render = function() {
      var node,
        _this = this;
      node = this.tag_d3('mainsvg');
      if (node === null) {
        node = d3.select(this.el).append('svg').attr('id', this.tag_id('mainsvg'));
        node.append('g').attr('id', this.tag_id('visualization'));
      }
      node.attr('width', this.mget('outerwidth')).attr("height", this.mget('outerheight'));
      this.tag_d3('visualization').attr('transform', _.template('translate(0, {{h}}) scale(1, -1)', {
  ...
  PlotView = (function(_super) {
    ...
    PlotView.prototype.render_mainsvg = function() {
      var node;
      node = this.tag_d3('plot');
      if (node === null) {
        node = d3.select(this.el).append('g').attr('id', this.tag_id('plot'));
      }
      node.attr('width', this.mget('outerwidth')).attr("height", this.mget('outerheight'));
  ...
  Bokeh.scatter_plot = function(parent, data_source, xfield, yfield, color_field, mark, colormapper) {
    var color_mapper, context, contextview, plot_model, plot_view, scatter_attrs, scatter_plot, source_name, xaxis, xmapper, yaxis, ymapper;
    ...
    contextview = new GraphicsContextView({
      'model': context
    });
    contextview.render();
