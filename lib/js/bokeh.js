(function() {
  ...
  BokehView = (function(_super) {
    ...
    BokehView.prototype.tag_el = function(tag) {
      return this.$el.find("#" + this.tag_id(tag));
    };
  ...
  PlotView = (function(_super) {
    ...
    PlotView.prototype.render_mainsvg = function() {
      var node;
      node = this.tag_d3('mainbox');
      if (node === null) {
        node = d3.select(this.el).append('svg').attr('id', this.tag_id('mainsvg'));
      }
      return node.attr('width', this.mget('width')).attr("height", this.mget('height'));
    };

    PlotView.prototype.render_mainbox = function() {
      var node;
      node = this.tag_d3('mainbox');
      if (node === null) {
        node = this.tag_d3('mainsvg').append('rect').attr('id', this.tag_id('mainbox'));
      }
      return node.attr('fill', this.model.get('background-color')).attr('stroke', this.model.get('foreground-color')).attr('width', this.mget('width')).attr("height", this.mget('height'));
    };
