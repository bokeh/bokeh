(function() {
  ...
  PlotView = (function(_super) {
    ...
    PlotView.prototype.render_mainsvg = function() {
      ...
      if (node === null) {
        ...
        node.append('g').attr('id', this.tag_id('center')).append('g').attr('id', this.tag_id('flipY')).append('g').attr('id', this.tag_id('plotcontent'));
      }
    ...
    PlotView.prototype.render_frames = function() {
      ...
      if (innernode === null) {
        innernode = this.tag_d3('center').insert('rect', this.tag_selector('flipY')).attr('id', this.tag_id('innerbox'));
        outernode = this.tag_d3('mainsvg').append('rect').attr('id', this.tag_id('outerbox'));
      }
  ...
  D3LinearAxisView = (function(_super) {
    ...
    D3LinearAxisView.prototype.render = function() {
      ...
      base = this.tag_d3('center', this.plot_id);
      node = this.tag_d3('axis');
      if (!node) {
        node = base.insert('g', this.tag_selector('flipY', this.plot_id)).attr('id', this.tag_id('axis')).attr('class', 'D3LinearAxisView').attr('stroke', this.mget('foreground_color'));
      }
      ...
      node.call(axis);
      node.selectAll('.tick').attr('stroke', this.mget('tick_color'));
  ...
  Bokeh.scatter_plot = function(parent, data_source, xfield, yfield, color_field, mark, colormapper) {
    var color_mapper, plot_attrs, plot_model, plot_view, scatter_attrs, scatter_plot, source_name, xaxis, xmapper, yaxis, ymapper;
    ...
    plot_attrs = plot_model = Collections['Plot'].create({
      data_sources: {
