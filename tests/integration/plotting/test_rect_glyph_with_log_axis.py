from __future__ import absolute_import

from bokeh.io import save
from bokeh.plotting import figure
from bokeh.models.sources import ColumnDataSource
from tests.integration.utils import has_no_console_errors

import pytest
pytestmark = pytest.mark.integration

@pytest.mark.screenshot
def test_rect_rendering_with_log_axis(output_file_url, selenium, screenshot):

    plot = figure(x_range=(0,30), y_range=(1,100), y_axis_type="log")

    x = [10, 20]
    y = [10, 20]

    sc_source = ColumnDataSource(data=dict(x=x, y=y))
    plot.scatter(x='x', y='y', size=20, color='black', source=sc_source)

    source = ColumnDataSource(data=dict(x=[(x[0]+x[1])/2], y=[(y[0]+y[1])/2], width=[x[1]-x[0]], height=[y[1]-y[0]]))
    plot.rect(x='x', y='y', width='width', height='height', source=source)

    # Save the plot and start the test
    save(plot)
    selenium.get(output_file_url)
    assert has_no_console_errors(selenium)
    assert screenshot.is_valid()
