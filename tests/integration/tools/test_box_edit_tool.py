...
def _make_plot(dimensions="both", num_objects=0):
    ...
    tool = BoxEditTool(
        dimensions=dimensions, num_objects=num_objects, renderers=[renderer]
    )
    plot.add_tools(tool)
    plot.toolbar.active_multi = tool
    code = (
        RECORD("x", "source.data.x")
        + RECORD("y", "source.data.y")
        + RECORD("width", "source.data.width")
...
def _make_server_plot(expected, num_objects=0):
    def modify_doc(doc):
        ...
        tool = BoxEditTool(
            dimensions="both", num_objects=num_objects, renderers=[renderer]
        )
        plot.add_tools(tool)
        plot.toolbar.active_multi = tool
        div = Div(text="False")
