...
"""
    ...
    def test_patch_bad_slice_indices(self):
        ...
        with pytest.raises(
            ValueError,
            match=r"Patch slices must have non-negative \(start, stop, step\) values, got slice\(1, 10, -1\)",
        ):
    ...
    def test_data_column_lengths(self):
        ...
        assert (
            str(warns[0].message)
            == "ColumnDataSource's columns must be of the same length. Current lengths: ('a', 2), ('b', 3)"
        )
        ...
        assert (
            str(warns[0].message)
            == "ColumnDataSource's columns must be of the same length. Current lengths: ('a', 3), ('b', 2)"
        )
